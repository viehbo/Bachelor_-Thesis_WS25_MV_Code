...
It allows users to precisely assess whether a dataset is suitable for a given task and, if so, which temporal resolution and spatial extent are necessary. The use case in Chapter~\ref{ch:use_case} illustrated this concretely: the exploration of wind variables revealed that the available 10\,m components in ERA5 hourly data on single levels are unlikely to carry predictive information for glacier mass balance, while
